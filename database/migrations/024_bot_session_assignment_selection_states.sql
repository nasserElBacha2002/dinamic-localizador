...
--
-- Safety notes:
-- - Constraint/index names match prior migrations (003, 007, 009, 014).
-- - DROP statements are guarded with IF EXISTS; missing objects do not fail the migration.
-- - CREATE UNIQUE INDEX will fail if duplicate active sessions exist for the same employee
--   (or employee + simulation_session_id). Resolve duplicates before applying in production.
