...
describe("inventory lifecycle", () => {
  ...
  it("resolves SCHEDULED before start", () => {
    assert.equal(
      resolveLifecycleInventoryStatus(baseInventory, new Date("2026-06-22T14:00:00.000Z")),
      "SCHEDULED",
    );
  });
  ...
  it("resolves IN_PROGRESS after start and before end", () => {
    assert.equal(
      resolveLifecycleInventoryStatus(baseInventory, new Date("2026-06-22T15:00:00.000Z")),
      "IN_PROGRESS",
    );
  });
  ...
  it("resolves COMPLETED after scheduled end", () => {
    assert.equal(
      resolveLifecycleInventoryStatus(baseInventory, new Date("2026-06-23T02:00:00.000Z")),
      "COMPLETED",
    );
  });
  ...
  it("keeps terminal statuses unchanged", () => {
    assert.equal(
      resolveLifecycleInventoryStatus(
        { ...baseInventory, status: "CANCELLED" },
