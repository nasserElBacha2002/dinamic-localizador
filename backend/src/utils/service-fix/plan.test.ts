...
import type { CurrentDbService, EnvironmentSnapshot, FixPlanOptions } from "./types";
...
const environmentSnapshot: EnvironmentSnapshot = {
  ...
  totalNumericCurrentDbServices: 2,
  totalNonNumericCurrentDbRows: 0,
  duplicateNumericServiceGroups: 0,
...
describe("buildFixPlan", () => {
  it("proposes coordinate fixes only for recomputed distances above 300m by default", () => {
    ...
    assert.equal(autoFixes[0]?.serviceNumber, "163");
  ...
  it("skips fixes when the service is missing in the current database", () => {
    const currentDb = buildCurrentDbState([]);
    ...
    assert.equal(plan.skipped[0]?.skippedReason, "current_service_not_found");
  });
