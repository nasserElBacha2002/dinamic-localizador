...
function buildPreviewColumns(isClientFormat: boolean): DataTableColumn<InventoryImportPreviewRow>[] {
  const columns: DataTableColumn<InventoryImportPreviewRow>[] = [
    ...
    { key: "storeName", header: "Tienda resuelta", getValue: (row) => row.storeName ?? "—" },
    { key: "rawFecha", header: isClientFormat ? "Fecha original" : "Inicio original", getValue: (row) => row.rawFecha || "—" },
  ];

  if (isClientFormat) {
    columns.push({
      key: "parsedInventoryDate",
      header: "Fecha inventario",
      getValue: (row) => row.parsedInventoryDate ?? "—",
    });
  }
