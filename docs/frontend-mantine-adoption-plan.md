...
**Status:** `COMPLETE` — Mantine migration + navigation polish (PR 20). Ready for PR to main.  
**Date:** 2026-06-23 · **Last updated after PR 20**  
...
### PR 12 — Complex forms + maps ✅ IMPLEMENTED

**Goal:** Migrate store create/edit flow and location/map UI to Mantine design-system without changing geofencing, Google Maps behavior, or API contracts.

**Scope (this PR only):** Store complex form + location picker. No bot simulator, statistics, import, or inventory detail.

**Migrated:**

| Area | Status |
|------|--------|
| `StoreCreatePage` | ✅ design-system `PageHeader`; no MUI |
| `StoreEditPage` | ✅ design-system `PageHeader`; loading/error/success unchanged |
| `StoreForm` | ✅ `FormSection` (Datos generales + Ubicación via picker), `FormGrid`, `FormActions`, RHF fields |
| `StoreLocationPicker` | ✅ `FormSection` Ubicación, responsive `SimpleGrid` |
| `ManualCoordinatesFields` | ✅ Mantine `Paper`, `TextInput`, `NumberInput`, `FormGrid` |
| `LocationMapSection` | ✅ Split into `LocationAddressSearch` + `LocationMapCanvas`; Mantine wrappers only |

**Preserved (unchanged):**

- `useLocationPickerState` hook — Google Maps loader, Places autocomplete, marker, circle, geocoding
- `store.schema.ts`, `useStores` hooks, submit payloads, navigation, default radius (150 m), lat/lng precision
- Route paths, permissions, company context

**New file:** `location-map-section.module.css` — styles `gmp-place-autocomplete` host only (visual).

**Not in PR 12:** `CompanySettingsPage`, statistics, bot simulator, import, detail dialogs (other PRs).

**Next:** PR 13 — Inventory detail / operational command center.

**Validation:** `npm run lint`, `npm run build`, `npm test` pass (126/126). Zero MUI in store form/location-picker target files.

**PR 12 correction (layout):** Store create/edit refactored to **left/right two-column layout** (40/60): left = Información general + Geolocalización + actions; right = Mapa interactivo (search inside map panel). Mobile stacks: info → map → geo → actions. Header actions on desktop via `form={STORE_FORM_ID}`.

**Next:** PR 13 — Inventory detail / operational command center.

---

### PR 13 — Inventory detail / operational command center ✅ IMPLEMENTED

**Goal:** Migrate inventory detail page to Mantine command-center layout without changing APIs or business logic.

**Migrated:**

| Area | Status |
|------|--------|
| `InventoryDetailPage` | ✅ `PageHeader`, `MetricCard` row, two-column grid, `SectionCard`, design-system states |
| `InventoryOperationalSection` | ✅ `SectionCard`, design-system `DataTable`, `StatusBadge`, `PaginationControls` |
| `InventoryDetailFieldGrid` | ✅ Mantine read-only field grid (new) |
| `InventoryForm` | ✅ `embedded` + `hideActions` + `formId` for inline edit on detail page |

**Preserved:** all queries, mutations, permissions, assign/unassign, review attendance, cancel operation, edit payloads, navigation.

**Deferred:** `ReviewAttendanceDialog` (already Mantine from prior PR), `AttendanceDetailPage`/`AbsenceDetailPage` shells (PR 17).

**Validation:** lint/build/test pass; zero MUI in target files.

**Next:** PR 14 — Statistics migration.

---

### PR 14 — Statistics migration ✅ IMPLEMENTED

**Goal:** Migrate statistics/analytics area to Mantine/design-system without changing APIs, chart data, or exports.

**Corrections (PR 12/13 cleanup):**

- `InventoryForm` — `FormGrid.Full` rows, shared timezone caption, aligned date/number pairs
- `InventoryOperationalSection` — domain-specific status tone helpers (`validationStatusTone`, `locationStatusTone`, etc.)
- `FormGrid.Full` — full-width span support in design-system

**Migrated:**

| Area | Status |
|------|--------|
| `StatisticsPage` | ✅ `PageHeader`, Mantine `Tabs`, `Stack` layout |
| `StatisticsFiltersBar` | ✅ design-system `FilterBar` (prior PR; verified) |
| `StatisticsGeneralTab` | ✅ `MetricCard` KPIs, `ChartCard`, Mantine `Grid` |
| Statistics tables | ✅ design-system `DataTable` + `PaginationControls` |
| `ChartCard` | ✅ `SectionCard` wrapper; ECharts data unchanged |
| `ExportActionButtons` | ✅ Mantine (preserved CSV/Excel behavior) |

**Preserved:** statistics hooks, query params, sorting, pagination, ECharts options/mapping, export filenames and payloads.

**Validation:** lint/build/test pass; zero MUI in statistics target files.

**Next:** PR 16 — Import flow migration.

---

### PR 15 — Bot simulator migration ✅ IMPLEMENTED

**Goal:** Migrate bot simulator to Mantine testing console without changing APIs or simulation logic.

**Migrated:**

| Area | Status |
|------|--------|
| `BotSimulatorPage` | ✅ 3-column console layout (`bot-simulator-console.module.css`) |
| `BotSessionPanel` | ✅ design-system `SectionCard` context panel |
| `BotConversationPanel` | ✅ WhatsApp-like chat, `ScrollArea`, `BotQuickActions` |
| `BotTechnicalDetails` | ✅ `SectionCard` + Mantine `Tabs` + `Code` blocks |
| `ChatBubble` | ✅ Mantine message bubbles (user right / bot left) |
| `BotLocationDialog` | ✅ Mantine `Modal` (already migrated; verified) |

**Preserved:** `useBotSimulatorSession`, all mutations, location presets, geofence simulation payloads, quick actions, JSON export, `datetime-local` input.

**Validation:** lint/build/test pass; zero MUI in bot simulator target files.

**Next:** PR 19 — Final MUI cleanup / remove MUI packages.

---

### PR 16 — Import flow migration ✅ IMPLEMENTED

**Goal:** Migrate inventory import wizard to Mantine/design-system without changing parsing, validation, or API payloads.

**Migrated:**

| Area | Status |
|------|--------|
| `InventoryImportPage` | ✅ `PageHeader`, step-based `SectionCard` layout |
| File upload | ✅ Mantine `Button` + native file input |
| Template/errors download | ✅ Mantine actions preserved |
| Preview table | ✅ design-system `DataTable` |
| Errors table | ✅ design-system `DataTable` + `StatusBadge` |
| Confirm section | ✅ Mantine `Alert` + actions |
| States | ✅ `LoadingState`, `ErrorState`, `EmptyState` |

**Preserved:** CSV/XLSX parsing, template download, validation messages, confirm payload, navigation to `/inventories`.

**Validation:** lint/build/test pass; zero MUI in import target files.

---

### PR 17 — Complex dialogs / review flows ✅ IMPLEMENTED

**Goal:** Migrate remaining complex dialogs and detail review shells to Mantine.

**Migrated:**

| Area | Status |
|------|--------|
| `AbsenceDetailPage` | ✅ design-system layout + `DataTable`; Mantine review `Modal` |
| `AttendanceDetailPage` | ✅ design-system layout + review history `DataTable`; Mantine `Accordion` |
| `EmployeeAbsenceBalanceCard` | ✅ design-system `DataTable` + Mantine edit `Modal` |
| `DetailFieldGrid` | ✅ Mantine `SimpleGrid` (no MUI) |

**Verified already Mantine (prior PRs):** `ReviewAttendanceDialog`, `CompanyUserDialog`, `CreatePlatformCompanyDialog`, `BotLocationDialog`.

**Preserved:** approve/reject flows, mutation payloads, permissions, review rules.

**Validation:** lint/build/test pass; zero MUI in migrated detail/review targets.

---

### PR 18 — Company settings + MUI cleanup preparation ✅ IMPLEMENTED

**Goal:** Migrate company settings UI and prepare for final MUI removal.

**Migrated:**

| Area | Status |
|------|--------|
| `CompanySettingsPage` | ✅ Section cards: general, geofence, tolerances, checkout, corrections, modules |
| Save/reset | ✅ Mantine buttons; `notifications` for success |
| States | ✅ design-system `LoadingState` / `ErrorState` |

**Cleanup:**

| Action | Status |
|--------|--------|
| Removed unused `AdminLayout.tsx` | ✅ Zero imports confirmed |
| `EmployeeEditPage` | ✅ `SectionCard` layout; notifications |

**Remaining MUI (documented for PR 19):** legacy `components/common/*` wrappers, `ModuleRouteGuard`/`FeatureRouteGuard`, `DateRangeCalendar`, `StatusCard`, `main.tsx` ThemeProvider.

**Validation:** lint/build/test pass; zero MUI in settings target files.

**Next:** PR 19 — Final MUI cleanup / remove packages.
...
| PR 13 | Inventory detail / operational command center ✅ |
| PR 14 | Statistics migration ✅ |
| PR 15 | Bot simulator migration ✅ |
| PR 16 | Import flow migration ✅ |
| PR 17 | Complex dialogs / review flows ✅ |
| PR 18 | Company settings + MUI cleanup prep ✅ |
| PR 19 | Final MUI cleanup + login + palette ✅ |
| PR 20 | Final QA / visual polish / navigation UX ✅ |

---

### PR 19 — Final MUI cleanup + login + Modern Professional palette ✅ IMPLEMENTED

**Goal:** Fix PR 16–18 review issues, remove all MUI usage and packages, apply Modern Professional palette, redesign login.
...
| Primary | `#1275F2` |
| Secondary | `#5F78A3` |
| Tertiary / accent | `#C55B00` |
| Neutral | `#74777F` |

**Corrections (PR 16–18 follow-up):**

- `InventoryImportPage` — invalid file clears preview and `selectedFileName`
- `AbsenceDetailPage` — modal comment reset; `inventoryStatusLabels` for affected inventories

**Migrated / removed:**

| Area | Status |
|------|--------|
| `LoginPage` | ✅ Split brand panel + form card; `PasswordInput` toggle |
| Mantine theme/tokens | ✅ Modern Professional palette |
| `DateRangeCalendar` | ✅ Mantine implementation in design-system |
| Route guards | ✅ Mantine `SectionCard` fallbacks |
| Legacy `components/common/*` MUI wrappers | ✅ Deleted |
| `main.tsx` | ✅ MUI `ThemeProvider` removed |
| `package.json` | ✅ `@mui/material`, `@emotion/*` removed |

**Validation:** lint/build/test pass; **0** MUI imports; packages removed.

---

### PR 20 — Final QA / visual polish / navigation UX ✅ IMPLEMENTED

**Goal:** Polish app shell navigation before opening PR to main.

**Changes:**

| Area | Status |
|------|--------|
| Sidebar sections | ✅ General, Operación, Gestión, Herramientas, Configuración |
| `CompanySwitcher` | ✅ Menu-based switcher with role labels |
| Topbar | ✅ Product title, compact user menu, logout in menu |
| Import nav link | ✅ `/inventories/import` in Herramientas (`inventories:manage`) |
| Login polish | ✅ Card shadow refinement |

**Preserved:** routes, permissions, module filtering, `selectCompany`, query cache clear, logout.

**Validation:** lint/build/test pass (128/128).
