...
export const monthlyAttendanceReportRunRepository = {
  async ensure(companyId: string, year: number, month: number, timezone: string): Promise<MonthlyReportRun> {
    ...
    try {
      ...
      if (!raced.recordset[0]) throw new Error("MONTHLY_REPORT_RUN_CREATE_FAILED", { cause: error });
      return map(raced.recordset[0]);
    }
  },
  async claim(id: string, companyId: string, owner: string, leaseSeconds: number): Promise<MonthlyReportRun | null> { const r = await getPool().request().input("id",sql.UniqueIdentifier,id).input("companyId",sql.UniqueIdentifier,companyId).input("owner",sql.NVarChar(100),owner).input("lease",sql.Int,leaseSeconds).query(";WITH next_row AS (SELECT TOP(1) id FROM monthly_attendance_report_runs WITH(UPDLOCK,READPAST,ROWLOCK) WHERE id=@id AND company_id=@companyId AND ((status IN(N'PENDING',N'PARTIAL',N'FAILED') AND (next_attempt_at IS NULL OR next_attempt_at<=SYSUTCDATETIME())) OR (status=N'PROCESSING' AND lease_expires_at<SYSUTCDATETIME()))) UPDATE r SET status=N'PROCESSING',attempt_count=attempt_count+1,lease_owner=@owner,lease_expires_at=DATEADD(SECOND,@lease,SYSUTCDATETIME()),updated_at=SYSUTCDATETIME() OUTPUT INSERTED.* FROM monthly_attendance_report_runs r JOIN next_row n ON n.id=r.id"); return r.recordset[0] ? map(r.recordset[0]) : null; },
  ...
  async mark(id:string, companyId:string, owner:string, status:string, error:string|null=null, nextAttemptAt: Date|null=null): Promise<void> { await getPool().request().input("id",sql.UniqueIdentifier,id).input("companyId",sql.UniqueIdentifier,companyId).input("owner",sql.NVarChar(100),owner).input("status",sql.NVarChar(30),status).input("error",sql.NVarChar(1000),error).input("nextAttemptAt",sql.DateTime2,nextAttemptAt).query("UPDATE monthly_attendance_report_runs SET status=@status,last_error=@error,next_attempt_at=@nextAttemptAt,lease_owner=NULL,lease_expires_at=NULL,sent_at=CASE WHEN @status=N'SENT' THEN SYSUTCDATETIME() ELSE sent_at END,updated_at=SYSUTCDATETIME() WHERE id=@id AND company_id=@companyId AND lease_owner=@owner"); },
};
