...
export interface ServiceLocationFields {
  address?: string;
  neighborhood?: string;
  locality?: string;
  latitude: number;
...
export const applyPlaceSelection = (
  current: ServiceLocationFields,
  place: PlaceSelectionResult,
  currentName?: string,
): { fields: ServiceLocationFields; state: LocationPickerState } => {
  const fields: ServiceLocationFields = {
    ...current,
...
export const applyManualCoordinates = (
  ...
  latitude: number,
  longitude: number,
): { fields: ServiceLocationFields; state: LocationPickerState } | null => {
