...
export async function getEmployees(
  ...
  const { data } = await apiClient.get<PaginatedResponse<Employee>>(companyApiPath("employees", companyId), {
    params: buildParams(filters as Record<string, string | number | boolean | undefined>),
  });
  return data;
}

export async function getEmployeeById(id: string, companyId?: string): Promise<Employee> {
  const { data } = await apiClient.get<SingleResponse<Employee>>(
    companyApiPath(`employees/${id}`, companyId),
  );
  return data.data;
}
