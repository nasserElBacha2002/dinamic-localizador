...
export type OperationImportScheduleDefaults = {
...
export const createOperationImportDateTimeUtils = (
  defaults: OperationImportScheduleDefaults,
) => {
  ...
  const parseOperationImportDateTime = (
    raw: string,
  ): { iso: string } | { error: string } => {
    const parsed = parseOperationImportDateValue(raw);
    if ("error" in parsed) {
  ...
  const buildClientInventorySchedule = (
    ...
    | { error: string } => {
    const parsed = parseOperationImportDateValue(rawFecha);
    if ("error" in parsed) {
      return { error: parsed.error };
    }
