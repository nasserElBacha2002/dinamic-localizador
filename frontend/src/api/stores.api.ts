...
export async function getStores(filters: StoreFilters = {}): Promise<PaginatedResponse<Store>> {
  const { data } = await apiClient.get<PaginatedResponse<Store>>(companyApiPath("/stores"), {
...
export async function getStoreById(id: string): Promise<Store> {
  const { data } = await apiClient.get<SingleResponse<Store>>(companyApiPath(`/stores/${id}`));
  return data.data;
}
