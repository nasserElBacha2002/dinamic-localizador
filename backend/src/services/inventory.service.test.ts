...
describe("inventoryService.create", () => {
  ...
  it("uses company defaults when tolerances are omitted", async () => {
    ...
    mock.method(storeRepository, "findById", async () => activeStore);
    mock.method(inventoryRepository, "create", async (_companyId, input) => ({
      ...createdInventory,
      earlyToleranceMinutes: input.earlyToleranceMinutes!,
      lateToleranceMinutes: input.lateToleranceMinutes!,
    }));

    const result = await inventoryService.create(COMPANY_ID, {
      storeId: STORE_ID,
      scheduledStart: FUTURE_START,
      scheduledEnd: FUTURE_END,
    });
  ...
  it("rejects negative tolerances at schema validation layer", async () => {
    setupUnitTestEnv();
    const { createInventorySchema } = await import("../schemas/inventory.schema");
  ...
  it("rejects inactive stores before resolving defaults", async () => {
    ...
    mock.method(storeRepository, "findById", async () => ({ ...activeStore, active: false }));

    await assert.rejects(
      () =>
        inventoryService.create(COMPANY_ID, {
          storeId: STORE_ID,
          scheduledStart: FUTURE_START,
          scheduledEnd: FUTURE_END,
        }),
