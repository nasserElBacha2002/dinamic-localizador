 ...
 *
 * In-memory only: adequate for single-instance; multi-instance needs shared store.
 */
...
export function rateLimitInvitations(options: {
  scope: string;
  windowMs: number;
  max: number;
}) {
...
export function resetInvitationRateLimitBucketsForTests(): void {
  buckets.clear();
  lastSweepAt = 0;
}
