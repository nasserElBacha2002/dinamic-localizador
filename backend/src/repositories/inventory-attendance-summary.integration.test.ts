    ...
    await pool
      ...
      .query(`
        INSERT INTO operation_assignments (
          company_id, inventory_id, employee_id, confirmation_status, confirmed_at, unavailable_at
        )
        VALUES
          (@companyId, @inventoryId, @pendingId, 'PENDING', NULL, NULL),
          (@companyId, @inventoryId, @confirmedId, 'CONFIRMED', SYSUTCDATETIME(), NULL),
          (@companyId, @inventoryId, @unavailableId, 'UNAVAILABLE', NULL, SYSUTCDATETIME())
      `);
