/*
  ...
  - Add nullable employee_workday_id + tenant FK
  - Filtered UQ on (employee_workday_id, notification_type, schedule_version)
  - Filtered UQ for legacy NULL-EW rows
  - Drop pre-Phase-3 operation/employee UQs only after EW UQ exists

  IMPORTANT: ADD COLUMN must be in its own GO batch before any statement that
  references employee_workday_id (SQL Server metadata binding). Atomicity for
  the whole script is owned by applySqlScriptInTransaction (Node TDS TX) —
  do not nest T-SQL BEGIN TRANSACTION here (error 266 with the runner).
...
/* Column must be visible in a separate batch before FK/index references. */
...
IF COL_LENGTH(N'dbo.whatsapp_attendance_notifications', N'employee_workday_id') IS NULL
BEGIN
    THROW 50132, 'employee_workday_id column missing after ADD', 1;
END;
GO
...
/* Drop legacy UQs only after EW UQ exists; recreate null-EW UQ in later batches.
